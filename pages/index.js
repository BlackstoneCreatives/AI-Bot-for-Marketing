...
export default function Chat() {
  ...
  const sendMessage = async (e) => {
    ...
    // Assuming there's an endpoint to get the AI response
    try {
    ...
    <div style={{ fontFamily: 'Arial, sans-serif', backgroundColor: '#f5f7fa', minHeight: '100vh', padding: '20px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto', backgroundColor: '#ffffff', borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px', height: '70vh', overflowY: 'auto' }}>
          {messages.map((msg, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: msg.role === 'user' ? 'flex-end' : 'flex-start', marginBottom: '10px' }}>
              <div style={{ backgroundColor: msg.role === 'user' ? '#2b6cb0' : '#edf2f7', color: msg.role === 'user' ? '#ffffff' : '#2d3748', padding: '12px 18px', borderRadius: '20px', maxWidth: '70%', fontSize: '1rem', lineHeight: '1.5', whiteSpace: 'pre-wrap' }}>
                {msg.text}
              </div>
            </div>
          ))}
        ...
        <form onSubmit={sendMessage} style={{ padding: '20px', borderTop: '1px solid #e2e8f0', display: 'flex', gap: '10px', backgroundColor: '#f8fafc' }}>
          ...
          />
          <button type="submit" disabled={loading} style={{ padding: '12px 20px', backgroundColor: '#2b6cb0', color: '#fff', borderRadius: '8px', border: 'none', fontWeight: '600', cursor: 'pointer' }}>
            Send
          </button>
        </form>
      </div>
