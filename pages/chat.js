...
export default function Chat() {
  const [messages, setMessages] = useState([
    {
      role: 'bot',
      text: "Hi there! I'm your campaign assistant. Let's build your ad strategy together. What's your organization’s name?",
    },
  ]);
  ...
  const runAudit = async (url) => {
    try {
      ...
      const auditMessage = {
        role: 'bot',
        text: data.result || 'Could not analyze the page.',
      };
...
try {
  const res = await fetch('/api/chat', {
    ...
    body: JSON.stringify({ message: input.trim() }),
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'bot', text: '⚠️ Error reaching the AI. Please try again.' },
      ]);
    } finally {
      setLoading(false);
    }
